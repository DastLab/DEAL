...
function App() {
  ...
  useEffect(() => {
    ...
    axios
      .get(process.env.REACT_APP_BACKEND+'/files')
      .then((response) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await axios.post(process.env.REACT_APP_BACKEND+'/upload', formData, {
        headers: {
      ...
      const filesResponse = await axios.get(process.env.REACT_APP_BACKEND+'/files');
  ...
  const handleEditFileName = async (fileId, currentFileName) => {
    ...
    if (newName) {
      ...
      try {
        ...
        const putResponse = await axios.put(`${process.env.REACT_APP_BACKEND}/files/${fileId}`, {
  ...
  const handleDelete = (fileId, event) => {
    ...
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`${process.env.REACT_APP_BACKEND}/files/${fileId}`)
          .then((response) => {
