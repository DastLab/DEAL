...
const { gemmaHandleUserInput} = require ('./gemmaapi.js');
...
const FileDetailsPage = ({ fileName, onBack }) => {
  ...
  const initialPrompts = useMemo(() => ({
    ...
    'Stats': "I also would like an overall summary that explains how these statistics interact with each other, "+
      "and their impact on dependencies, how these values are distributed across different headers and the significance of this distribution for an overall understanding of the dataset."
  }), []);
  ...
  const prompts = useMemo(() => ({
    'RFDs Overview':
      ...
      "determines another attribute (rhs - right-hand side), with specific tolerance thresholds indicated. The notation for an RFD is structured as follows: attribute_x@thr_x, attribute_y@thr_y, ... -> attribute_z@thr_z, " +
      "where for each attribute the tolerance threshold used to compare the similarity of the attribute values is indicated after the \"@\" character.. Provide an explanation of each attribute and dependency, based on the attribute names and associated tolerance thresholds, " +
  ...
  useEffect(() => {
    axios.get(`${process.env.REACT_APP_BACKEND}/files/${fileName}`)
      .then(response => {
  ...
  const extractLhsAndRhs = (data) => {
    const extractedRFDs = [];
      ...
      data.execution_result.result.data.forEach(resultData => {

            if (resultData.lhs && resultData.rhs) {


              const lhsColumns = resultData.lhs.map((lhsItem, idx) => `${lhsItem.column}@${lhsItem.comparison_relaxation}`).join(', ');
              const rhsColumns = `${resultData.rhs.column}@${resultData.rhs.comparison_relaxation}`
...
const filteredStatisticLabels = Object.keys(statistics.attribute_types).filter(label => !selectedHeaderValues.includes(label));
...
const statisticModes = filteredStatisticLabels.map(label => statistics.mode[label]);
  ...
  const totalChartsBoxPlot = Object.keys(statistics.attribute_types)
  ...
  const getPaginatedChartsBoxPlot = () => {
    const filteredLabels = Object.keys(statistics.attribute_types)
      .filter(label => !selectedHeaderValues.includes(label));
  ...
  const minMaxLabels = Object.keys(statistics.attribute_types).filter(label => !selectedHeaderValues.includes(label));
const statisticMin = minMaxLabels.map(label => statistics.min_values[label]);
const statisticMax = minMaxLabels.map(label => statistics.max_values[label]);
  ...
  const calculateNullPercentages = (distribution, rowNumbers) => {
    ...
    for (const col in distribution) {
      ...
      for (const key in colData) {
        ...
        if (key === "" || key === "?" || key === "null" || key === "NA" || key === "NaN") {
          nullCount += colData[key];
        }
      }

      const nullPercentage = rowNumbers ? ((nullCount / rowNumbers) * 100).toFixed(2) : 0;
      nullPercentages[col] = {
        nullCount,
        nullPercentage
      };
  ...
  const nullPercentages = calculateNullPercentages(statistics.distribution, info.row_number );
  ...
  const calculateRelativeFrequency = (distribution) => {
    ...
    Object.keys(distribution).forEach(attribute => {
      ...
      const total = Object.values(attributeCounts).reduce((sum, item) => sum + item, 0);
      relativeFrequency[attribute] = {};

      Object.keys(attributeCounts).forEach(value => {
        const count = attributeCounts[value];
  ...
  const distributionChartOptions = {
    ...
    plugins: {
      tooltip: {
        ...
        titleFont: { size: 0 },
        bodyFont: { size: 14, color: getColors().text },
        padding: 10,
        caretPadding: 5,
        caretSize: 5,
        cornerRadius: 4,
      ...
      <div className={'file-details'}>
        ...
        <div className="title-back-container">
          <div className="sidebar">
            <button className="sidebar-icon" data-title="Dataset"
                    onClick={() => handleMenuItemClick('dataset')}><DatabaseIcon/></button>
            <button className="sidebar-icon" data-title="Statistics"
                    onClick={() => handleMenuItemClick('statistics')}><BarChart/></button>
            <button className="sidebar-icon" data-title="Algorithm"
                    onClick={() => handleMenuItemClick('algorithm')}><PcIcon/></button>
            <button className="sidebar-icon" data-title="Dependency Analysis"
                    onClick={() => handleMenuItemClick('dependencies')}><ChartIcon/></button>
          </div>
        ...
        <div className="container">
                    ...
                    <div className="horizontal-scroll">
                      {header[0].map((item, index) => (
                          <div key={index} className="item-container">
                            ...
                            >
                              <span
                                  className={`details-header ${selectedHeaderValues.includes(item) ? 'selected' : ''}`}>{item}</span>
                            </button>
                            <div style={{height: '5px'}}></div>
                            <button
                                type="button"
                                className={`btn ${selectedHeaderValues.includes(statistics.attribute_types[item]) ? 'btn-group-toggle active' : 'btn btn-secondary'} no-pulse`}
                                style={{background: '#E1E1E1', cursor: "default"}}
                            >
                              <span className="details-header">{statistics.attribute_types[item]}</span>
                            </button>
                          </div>
                      ))}
                    </div>
                ...
                <div className="d-flex justify-content-between align-items-center card-header">
                  ...
                  <div className="toggle-button-cover">
                    <div id="button-3" className="button r">
                      <input className="checkbox" type="checkbox" onChange={() => toggleCardVisibility('details')}
                             checked={cardVisibility.details}/>
                    ...
                    <div className="card-body">
                      {info.size.map((item, index) => (
                              <div key={index}>
                                ...
                                <strong>Format:</strong> {info.file_format[index] !== null ? info.file_format[index] : 'N/A'} <br/>
              ...
              <div className="card mb-3">
                <div className="d-flex justify-content-between align-items-center card-header">
                  ...
                  <div className={`menu infoContentSpecifications ${menuOpenInfoContentSpecifications ? 'open' : ''}`}>
                    Provides three specifications on the contents of the dataset (number of columns, rows, character to
                    ...
                    <div className="card-body">
                      {info.col_number.map((item, index) => (
                              <div key={index}>
                                ...
                                <strong>Row:</strong> {info.row_number[index] !== null ? info.row_number[index] : 'N/A'}
                                <br/>
                                <strong>Blank
                                  char:</strong> {info.blank_char[index] !== null ? info.blank_char[index] : 'N/A'}
                              </div>
                          )
                      )}
                    </div>
          ...
          <h2 id="statistics" className="section">COLUMNS STATISTICS</h2>
          ...
          <div className="card mb-3">
                  ...
                  <div className="row">
                    {getPaginatedChartsBoxPlot().map((seriesItem, index) => (
                        <div key={index} className="col-md-6 mb-3">
                          <ReactApexChart options={options} series={[{type: 'boxPlot', data: [seriesItem]}]}
            ...
            <div className="pagination-container d-flex justify-content-center mt-3">
              <div className="pagination-bar">
                <button onClick={() => handlePageChangeBoxPlot(currentPageBoxPlot - 1)}
                        disabled={currentPageBoxPlot === 1} className="pagination-button">{'<'}</button>
                {renderPaginationButtonsBoxPlot()}
                <button onClick={() => handlePageChangeBoxPlot(currentPageBoxPlot + 1)}
                        disabled={currentPageBoxPlot === totalPagesBoxPlot} className="pagination-button">{'>'}</button>
              </div>
            </div>
          </div>
            ...
            <div className="col-md-12">
              <div className="card mb-3">
                <div className="d-flex justify-content-between align-items-center card-header">
                  ...
                  <div className="toggle-button-cover">
                    <div id="button-3" className="button r">
                      <input className="checkbox" type="checkbox" onChange={() => toggleCardVisibility('minmax')}
                             checked={cardVisibility.minmax}/>
                      <div className="knobs"></div>
                      <div className="layer"></div>
                    </div>
                  </div>
            ...
            <div className="col-md-12">
              <div className="card mb-3">
                <div className="d-flex justify-content-between align-items-center card-header">
                  ...
                  <div className="toggle-button-cover">
                    <div id="button-3" className="button r">
                      <input className="checkbox" type="checkbox" onChange={() => toggleCardVisibility('nullValues')}
                             checked={cardVisibility.nullValues}/>
                      <div className="knobs"></div>
                      <div className="layer"></div>
                    </div>
                  </div>
          ...
          <div className="card mb-3" style={{marginTop: 0}}>
            <div className="d-flex justify-content-between align-items-center card-header">
              ...
              <div className={`menu infoColumn ${menuOpenInfoColumn ? 'open' : ''}`}>
                Contains a pie chart for each column showing the percentage distribution of their values sorted from the
                most frequent ones.
              </div>
                                ...
                                {labels.map((label, index) => (
                                    <div key={index} className="label-box">
                                      <div className="color-box"
                                           style={{backgroundColor: backgroundColors[index]}}></div>
                                      {' ' + label}
                                    </div>
                                ))}
                              </div>
              ...
              <div className="card mb-3">
                <div className="d-flex justify-content-between align-items-center card-header">
                  ...
                  <div className={`menu infoSystem ${menuOpenInfoSystem ? 'open' : ''}`}>
                    Provides details about the system on which the algorithm was used for discovery (operating system,
                    processor, cores and threads, available RAM).
                  </div>

                  <div className="toggle-button-cover">
                    <div id="button-3" className="button r">
                      <input className="checkbox" type="checkbox" onChange={() => toggleCardVisibility('system')}
                             checked={cardVisibility.system}/>
                      <div className="knobs"></div>
                      <div className="layer"></div>
                    </div>
                  </div>
            ...
            <div className="col-md-6">
              ...
              <InfoIcon style={{width: '25px', height: '25px', marginTop: '3px'}}/>
                  ...
                  <div className={`menu infoExecutionParameters ${menuOpenInfoExecutionParameters ? 'open' : ''}`}>
                    Specifies the settings and conditions under which the algorithm was executed (command, max execution
                    time, max RAM usage, execution start time, execution end time).
                  </div>
                  ...
                  <div className="toggle-button-cover">
                    <div id="button-3" className="button r">
                      <input className="checkbox" type="checkbox"
                             onChange={() => toggleCardVisibility('executionParameters')}
                             checked={cardVisibility.executionParameters}/>
                      <div className="knobs"></div>
                      <div className="layer"></div>
                    </div>
                  </div>
              ...
              <div className={`card mb-3 w-100 ${cardVisibility.timeExecution ? 'h-100' : ''}`}>
                <div className="d-flex justify-content-between align-items-center card-header">
                  ...
                  <div className={`menu infoTimeExecution ${menuOpenInfoTimeExecution ? 'open' : ''}`}>
                    Provides an overview of the times taken to execute the various phases of the algorithm (dataset
                    loading, preprocessing, discovery, left, total).
                  </div>


                  <div className="toggle-button-cover">
                    <div id="button-3" className="button r">
                      <input className="checkbox" type="checkbox" onChange={() => toggleCardVisibility('timeExecution')}
                             checked={cardVisibility.timeExecution}/>
                      <div className="knobs"></div>
                      <div className="layer"></div>
                    </div>
                  </div>
                ...
                {cardVisibility.timeExecution && (
                    <div className="card-body">
                      {info.dataset_loading.map((item, index) => (
                          <div key={index}>
                            <strong>Dataset
                              Loading:</strong> {info.dataset_loading[index] !== null ? formatTime(info.dataset_loading[index]) : 'N/A'}
                            <br/>
                            <strong>Preprocessing:</strong> {info.preprocessing[index] !== null ? formatTime(info.preprocessing[index]) : 'N/A'}
                            <br/>
                            <strong>Discovery:</strong> {info.discovery[index] !== null ? formatTime(info.discovery[index]) : 'N/A'}
                            <br/>
                            <strong>Left:</strong> {formatTime(left) !== '0ms' && formatTime(left) !== '0s' ? formatTime(left) : 'N/A'}
                            <br/>
                            <strong>Total:</strong> {info.total[index] && info.total[index].trim() !== '' ? formatTime(info.total[index]) : 'N/A'}
                            <br/>
                          </div>
                      ))}
                    </div>
              ...
              <div className={`card mb-3 w-100 ${cardVisibility.ramUsage ? 'h-100' : ''}`}>
                <div className="d-flex justify-content-between align-items-center card-header">
                  ...
                  <div className={`menu infoRamUsage ${menuOpenInfoRamUsage ? 'open' : ''}`}>
                    Reports information about RAM usage during the execution of the algorithm (units of measurement, max
                    amount of RAM used).
                  </div>

                  <div className="toggle-button-cover">
                    <div id="button-3" className="button r">
                      <input className="checkbox" type="checkbox" onChange={() => toggleCardVisibility('ramUsage')}
                             checked={cardVisibility.ramUsage}/>
                      <div className="knobs"></div>
                      <div className="layer"></div>
                    </div>
                  </div>
                ...
                {cardVisibility.ramUsage && (
                    <div className="card-body">
                      {info.max_ram_used.map((item, index) => (
                          <div key={index}>
                            <strong>Unit:</strong> {info.unit[1] && info.unit[1].trim() !== '' ? info.unit[1] : 'N/A'}
                            <br/>
                            <strong>Max Ram
                              Used:</strong> {info.max_ram_used[index] !== null ? info.max_ram_used[index] : 'N/A'}
                            <br/>
                          </div>
                      ))}
                    </div>
              ...
              <div className={`card mb-3 w-100 ${cardVisibility.error ? 'h-100' : ''}`}>
                <div className="d-flex justify-content-between align-items-center card-header">
                  ...
                  <div className={`menu infoError ${menuOpenInfoError ? 'open' : ''}`}>
                    Shows any errors or anomalies found during the execution of the algorithm (time limit, memory limit,
                    general error).
                  </div>

                  <div className="toggle-button-cover">
                    <div id="button-3" className="button r">
                      <input className="checkbox" type="checkbox" onChange={() => toggleCardVisibility('error')}
                             checked={cardVisibility.error}/>
                      <div className="knobs"></div>
                      <div className="layer"></div>
                    </div>
                  </div>
                    ...
                    <div className="card-body">
                      {info.time_limit.map((item, index) => (
                          <div key={index}>
                            ...
                            <strong>Memory
                              Limit:</strong> {info.memory_limit[index] !== null ? info.memory_limit[index] : 'N/A'}
                            <br/>
                            <strong>General
                              Error:</strong> {info.general_error[index] && info.general_error[index].trim() !== '' ? info.info.general_error[index] : 'N/A'}
                            <br/>
                          </div>
                      ))}
                    </div>
          ...
          <div className="row">
              ...
              {shouldDisplayCard && (
                  ...
                  <InfoIcon style={{width: '25px', height: '25px', marginTop: '3px'}}/>
                      ...
                      <div className={`menu infoTimeExecution2 ${menuOpenInfoTimeExecution2 ? 'open' : ''}`}>
                        It is a single horizontal bar graph that reports in percentage all four timings of the execution
                        algorithm (dataset loading, preprocessing, discovery, left).
                      </div>

                      <div className="toggle-button-cover">
                        <div id="button-3" className="button r">
                          <input className="checkbox" type="checkbox"
                                 onChange={() => toggleCardVisibility('timeExecution2')}
                                 checked={cardVisibility.timeExecution2}/>
                          <div className="knobs"></div>
                          <div className="layer"></div>
                        </div>
                      </div>
            ...
            <h2 id="dependencies" className="section">DEPENDENCY ANALYSIS</h2>
              ...
              <div className="card mb-3">
                <div className="d-flex justify-content-between align-items-center card-header">
                  ...
                  <div className={`menu infoCardinality ${menuOpenInfoCardinality ? 'open' : ''}`}>
                    It is a bar chart that shows the amount of dependencies for each cardinality of the left side of a
                    relationship. Values ​​in the legend can be selected to filter RFDs.
                  </div>
          ...
          <div className="card mb-12">
            <div className="d-flex justify-content-between align-items-center card-header">
              ...
              <div className={`menu infoFrequency ${menuOpenInfoFrequency ? 'open' : ''}`}>
                It is a bar chart showing for each attribute the frequency of comparison thresholds. The values ​​in the
                legend can be selected to filter the RFDs.
              </div>
          ...
          <div className="card mb-3">
            <div className="d-flex justify-content-between align-items-center card-header">
              ...
              <div className={`menu infoImplicating ${menuOpenInfoImplicating ? 'open' : ''}`}>
                It is a bar chart that lists for each column the union of all the attributes that imply it. The values
                ​​in the legend can be selected to filter the RFDs.
              </div>
        ...
        <span className="details-text">
          RFDs (total: {allRFDs.length} - filtered: {filteredRFDs.length})
            ...
            <div className={`menu infoRFD ${menuOpenInfoRFD ? 'open' : ''}`}>
              A Relaxed Functional Dependency (RFD) is a relationship between attributes where the left-hand
              side<br/> (LHS) set of attributes determines the right-hand side (RHS)
              attribute, with specified tolerance thresholds.<br/> The notation follows this format: <i>attribute_x@thr_x,
              attribute_y@thr_y, ... -&gt; attribute_z@thr_z</i>, with the
              LHS<br/> attributes separated from the RHS attribute by the symbol -&gt; and each attribute is associated
              with a<br/> tolerance threshold indicated after the "@" character
            </div>
          ...
          <div className="card mb-3">
            ...
            <div className={`menu infoPrompt ${menuOpenInfoPrompt ? 'open' : ''}`}>
              There are two types of automatically generated prompts available. The first prompt solely requires
              an<br/> explanation and analysis of the selected dependencies.
              The second prompt, similar to the first, adds a<br/> request for an explanation of the dependencies while
              incorporating distribution values, such as the most<br/>
              common values for each column in the dataset, along with the mean, median, and mode. You can
              select<br/> from a dropdown menu one of the three available LLMs
